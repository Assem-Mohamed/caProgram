...
public class Ca {
    ...
    public Instruction decode(int instructionValue) {
        ...
        switch (opcode) {
            case 0:
                statement = "Instruction: ADD R" + r1 + " R" + r2 + " R" + r3;
                break;
            case 1:
                statement = "Instruction: SUB R" + r1 + " R" + r2 + " R" + r3;
                break;
            case 2:
                statement = "Instruction: MUL R" + r1 + " R" + r2 + " R" + r3;
                break;
            case 3:
                statement = "Instruction: MOVI R" + r1 + " " + r2 + " " + immediate;
                break;
            case 4:
                statement = "Instruction: JEQ R" + r1 + " R" + r2 + " " + immediate;
                break;
            case 5:
                statement = "Instruction: AND R" + r1 + " R" + r2 + " R" + r3;
                break;
            case 6:
                statement = "Instruction: XORI R" + r1 + " R" + r2 + " " + immediate;
                break;
            case 7:
                statement = "Instruction: JMP " + address;
                break;
            case 8:
                statement = "Instruction: LSL R" + r1 + " R" + r2 + " " + r3 + shamt;
                break;
            case 9:
                statement = "Instruction: LSR R" + r1 + " R" + r2 + " " + r3 + shamt;
                break;
            case 10:
                statement = "Instruction: MOVR R" + r1 + " R" + r2 + " " + immediate;
                break;
            case 11:
                statement = "Instruction: MOVM R" + r1 + " R" + r2 + " " + immediate;
                break;
            default:
                break;
        }

        return new Instruction(pcRegister, opcode, r1, r2, r3, shamt, immediate, address, valueR1, valueR2, valueR3,
                type, statement);
    ...
    public void execute(Instruction instruction) {
        ...
        if (instruction.type.equals('R')) {
            switch (instruction.opcode) {
                case 0:
                    instruction.valueR1 = instruction.valueR2 + instruction.valueR3;
                    break;
                case 1:
                    instruction.valueR1 = instruction.valueR2 - instruction.valueR3;
                    break;
                ...
                case 5:
                    ...
                    break;
                case 8:
                    instruction.valueR1 = instruction.valueR2 << instruction.shamt;
                    break;
                case 9:
                    instruction.valueR1 = instruction.valueR2 >>> instruction.shamt;
                    break;
            ...
            switch (instruction.opcode) {
                case 3:
                    instruction.valueR1 = instruction.immediate;
                    break;
                case 4:
                    ...
                    break;
                case 6:
                    instruction.valueR1 = instruction.valueR2 ^ instruction.immediate;
                    break;
                case 10:
                    break;
                case 11:
    ...
    public void mem(Instruction instruction) {
        if (instruction.opcode == 10) {
            ...
            System.out.println("Data Memory Block " + instruction.valueR2 + instruction.immediate + ": "
                    + memory.dataMemory.get(instruction.valueR2 + instruction.immediate));
        }
    }
    ...
    public void pipeline() {
        ...
        for (clk = 1; clk <= (7 + ((n - 1) * 2)); clk++) {
            ...
            if (fetching == null && decoding == null && executing == null && memorying == null
                    && writingbacking == null) {
                n = clk;
                break;
            }
            ...
            // printing :)
            HomeGui.cyclePrints.setText(HomeGui.cyclePrints.getText() + "\n" + "PC  " + (pcRegister) + "\n" + "Fetching = "
                    + ((fetching == null) ? "---" : fetching) + "\n" + "Decoding = "
                    + ((decoding == null) ? "---" : decoding) + "\n" + "Executing = "
                    + ((executing == null) ? "---" : executing) + "\n" + "Memory = "
                    + ((memorying == null) ? "---" : memorying) + "\n" + "Write Back = "
                    + ((writingbacking == null) ? "---" : writingbacking) + "\n"
                    + "-------------------------------------------------------" + "\n");
        ...
        for (int i = 0; i < registerFile.registers.size(); i++) {
            HomeGui.registerFile.setText(HomeGui.registerFile.getText() + "\n" + "Register: "
                    + registerFile.registers.get(i).name + "  " + " Value: " + registerFile.registers.get(i).value);
            System.out.println("Register: " + registerFile.registers.get(i).name + "  " + " Value: "
                    + registerFile.registers.get(i).value);
        }

        for (int i = 0; i < memory.dataMemory.size(); i++) {
            if (memory.dataMemory.get(i) != 0) {
                HomeGui.dataMemory.setText(HomeGui.dataMemory.getText() + "\n" + "Data Memory Block " + (i + 1024) + ": "
                        + memory.dataMemory.get(i));
                System.out.println("Data Memory Block " + (i + 1024) + ": " + memory.dataMemory.get(i));
            }

        }
        for (int i = 0; i < memory.instructionMemory.size(); i++) {
            HomeGui.instructionMemory.setText(HomeGui.instructionMemory.getText() + "\n" + "Instruction Memory Block " + (i) + ": "
                    + memory.instructionMemory.get(i));
